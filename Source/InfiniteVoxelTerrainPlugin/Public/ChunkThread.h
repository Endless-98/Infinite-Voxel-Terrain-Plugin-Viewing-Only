...
#include "ChunkThreadUtilities.h"

struct FChunkConstructionData;
struct FChunkMeshData;
class FChunkThread;

struct FChunkThreadPolicy
{
    TFunction<void(FChunkThread&, TArray<int16>&, const FVector2D&, TArray<int32>&)> GenerateHeightmap;
    TFunction<bool(FChunkThread&, TArray<uint8>&, const TArray<int16>&, const FVector&)> GenerateChunkVoxels;
    TFunction<void(FChunkThread&, FChunkMeshData&, TArray<uint8>&, const FIntVector, const bool)> GenerateChunkMeshData;
};

struct FChunkTrackerState
{
...
public:
    ...
    FChunkThread(
        ...
        TerrainHeightMultiplier(TerrainHeightMultiplier), TerrainNoiseScale(TerrainNoiseScale), BiomeNoiseScale(BiomeNoiseScale), LodNearDistanceInChunks(LodNearDistanceInChunks), LodFarDistanceInChunks(LodFarDistanceInChunks), LodDistanceCurveExponent(LodDistanceCurveExponent), MaxLodStepPower(MaxLodStepPower), bUseGreedyMeshing(bUseGreedyMeshing), Seed(Seed),
    ...
    int32 DetermineLodStepForChunk(const FVector& ChunkLocation) const;
    ...
    void GenerateChunkMeshDataFromEncoded(FChunkMeshData& OutChunkMeshData, const TArray<uint8>& EncodedVoxels, const FIntVector ChunkCell, const bool bShouldGenerateCollisionAtChunkSpawn);
    void SetPolicy(const TSharedPtr<FChunkThreadPolicy>& InPolicy) { Policy = InPolicy; }
...
protected:
    ...
    int32 TempGenerationRadius{ ChunkGenerationRadius };
    ...
    int32 ActiveChunkLodStep{ 1 };

    // Used by threads to determine which spot to generate next
    FChunkTrackerState TrackerState{};
    ...
    TSharedPtr<FChunkThreadPolicy> Policy{};
