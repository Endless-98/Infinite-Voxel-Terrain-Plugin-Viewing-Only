...
public:
	...
	FChunkConstructionData(FChunkConstructionData&& Other) noexcept
		...
		, LodStep(MoveTemp(Other.LodStep))
...
enum class EChunkWorldCommandType : uint8
{
	Spawn,
	DestroyOrHide,
	Unreplicate,
...
struct FChunkWorldCommand
{
	EChunkWorldCommandType Type{};
	FIntVector ChunkCell{};
	TSharedPtr<FChunkConstructionData> ConstructionData{};
	int32 ChunkGenRadius{};
	int32 CollisionGenRadius{};
	bool bShouldGenerateMesh{ true };

	static FChunkWorldCommand MakeSpawn(TSharedPtr<FChunkConstructionData> InData, int32 InChunkGenRadius, int32 InCollisionGenRadius, bool bInShouldGenerateMesh)
	{
		FChunkWorldCommand Command;
		Command.Type = EChunkWorldCommandType::Spawn;
		Command.ChunkCell = InData.IsValid() ? InData->Cell : FIntVector::ZeroValue;
		Command.ConstructionData = MoveTemp(InData);
		Command.ChunkGenRadius = InChunkGenRadius;
		Command.CollisionGenRadius = InCollisionGenRadius;
		Command.bShouldGenerateMesh = bInShouldGenerateMesh;
		return Command;
	}

	static FChunkWorldCommand MakeCellCommand(EChunkWorldCommandType InType, const FIntVector& InChunkCell)
	{
		FChunkWorldCommand Command;
		Command.Type = InType;
		Command.ChunkCell = InChunkCell;
		return Command;
	}
};
...
struct FTerrainSettings
{
	...
	UPROPERTY(BlueprintReadWrite, EditAnywhere, Category = "Terrain Settings")
	int32 VoxelCount{ 32 };
	UPROPERTY(BlueprintReadWrite, EditAnywhere, Category = "Terrain Settings|LOD")
	int32 LodNearDistanceInChunks{ 4 };
	UPROPERTY(BlueprintReadWrite, EditAnywhere, Category = "Terrain Settings|LOD")
	int32 LodFarDistanceInChunks{ 18 };
	UPROPERTY(BlueprintReadWrite, EditAnywhere, Category = "Terrain Settings|LOD")
	float LodDistanceCurveExponent{ 1.6f };
	UPROPERTY(BlueprintReadWrite, EditAnywhere, Category = "Terrain Settings|LOD")
	int32 MaxLodStepPower{ 3 };
	UPROPERTY(BlueprintReadWrite, EditAnywhere, Category = "Terrain Settings|Mesh")
	bool bUseGreedyMeshing{ true };

	FTerrainSettings() = default;

	FTerrainSettings(int32 InSeed, float InTerrainHeightMultiplier, float InBiomeNoiseScale, float InTerrainNoiseScale, float InFoliageNoiseScale, int32 InChunkDeletionBuffer, int32 InCollisionChunkRadius, float InVoxelSize, int32 InVoxelCount, int32 InLodNearDistanceInChunks, int32 InLodFarDistanceInChunks, float InLodDistanceCurveExponent, int32 InMaxLodStepPower, bool bInUseGreedyMeshing)
		...
		LodNearDistanceInChunks(InLodNearDistanceInChunks), LodFarDistanceInChunks(InLodFarDistanceInChunks), LodDistanceCurveExponent(InLodDistanceCurveExponent), MaxLodStepPower(InMaxLodStepPower), bUseGreedyMeshing(bInUseGreedyMeshing) {}
	...
	friend FArchive& operator<<(FArchive& Ar, FTerrainSettings& Settings)
	{
		...
		Ar << Settings.LodNearDistanceInChunks;
		Ar << Settings.LodFarDistanceInChunks;
		Ar << Settings.LodDistanceCurveExponent;
		Ar << Settings.MaxLodStepPower;
		Ar << Settings.bUseGreedyMeshing;
		return Ar;
	}
};
...
class FRegionDataService;
class FChunkReplicationService;
class FChunkManagerTrackingService;
class FChunkManagerRegionService;
...
public:
	...
	friend class FRegionDataService;
	friend class FChunkReplicationService;
	friend class FChunkManagerTrackingService;
	friend class FChunkManagerRegionService;
...
private: 
	...
	void UnhideChunksInHeightmapLocations(const TArray<FVector2D>& HeightmapLocations);
...
private:
	...
	float VoxelSize{ 100 };
	int32 VoxelCount{ 32 };
	UPROPERTY(BlueprintReadWrite, EditAnywhere, Category = "Generation Settings|LOD", meta = (AllowPrivateAccess = "true", ClampMin = "0"))
	int32 LodNearDistanceInChunks{ 4 };
	UPROPERTY(BlueprintReadWrite, EditAnywhere, Category = "Generation Settings|LOD", meta = (AllowPrivateAccess = "true", ClampMin = "1"))
	int32 LodFarDistanceInChunks{ 18 };
	UPROPERTY(BlueprintReadWrite, EditAnywhere, Category = "Generation Settings|LOD", meta = (AllowPrivateAccess = "true", ClampMin = "0.1"))
	float LodDistanceCurveExponent{ 1.6f };
	UPROPERTY(BlueprintReadWrite, EditAnywhere, Category = "Generation Settings|LOD", meta = (AllowPrivateAccess = "true", ClampMin = "0", ClampMax = "6"))
	int32 MaxLodStepPower{ 3 };
	UPROPERTY(BlueprintReadWrite, EditAnywhere, Category = "Generation Settings|Mesh", meta = (AllowPrivateAccess = "true"))
	bool bUseGreedyMeshing{ true };
	...
	TMap<FIntPoint, TMap<FIntVector, TMap<int32, uint8>>> ModifiedVoxelsByCellByRegion; 	// Sparse patches: only store modified indices
	...
	FCriticalSection WorldCommandMutex{};
	TArray<FChunkWorldCommand> PendingWorldCommands{};
	void EnqueueWorldCommand(FChunkWorldCommand&& Command);
